...
def cargar_dimensiones():
    ...
    # ── FASE 2: dim_cancion — DESPUÉS de que dim_usuario esté completa ────────
...
# ─────────────────────────────────────────────────────────────────
# PASO 5: hechos_adquisicion (CORREGIDO DINÁMICO)
# ─────────────────────────────────────────────────────────────────
def cargar_hechos_adquisicion():
    print("\n[5] Cargando hechos_adquisicion (incremental dinámico)...")
    conn_usr = dblink_conn("UsersETL")
    
    # FIX v5.2: Se mapea `src.created_at::DATE` con `dfh.fecha` para obtener el `id_fecha_hora` real.
    # En caso de que un registro no coincida con una fecha dimensional, se hace un COALESCE con 
    # la primera fecha disponible o el ID por defecto para evitar violaciones de integridad.
    ejecutar(URI_OLAP, f"""
    ...
    WITH base AS (
        ...
        FROM (
            ...
            WHERE dfh.fecha >= (
                ...
                JOIN dim_fecha_hora d ON h.id_fecha_hora = d.id_fecha_hora
            ) - INTERVAL '3 months'
...
def analizar_tablas():
    ...
    tareas = [
        (URI_OLAP, f"ANALYZE {t};", f"ANALYZE {t}")
        for t in tablas
    ]
    with ThreadPoolExecutor(max_workers=len(tareas)) as pool:
        futures = {pool.submit(_exec_dim, t): t[2] for t in tareas}
        for f in as_completed(futures):
            try:
                msg, _, secs = f.result()
                print(f"   [OK] {msg} → ? filas  ({secs:.1f}s)")
            except Exception as e:
...
if __name__ == "__main__":
    ...
    print("==================================================")
    print(f"    ETL EJECUTADO EXITOSAMENTE EN: {duracion:.2f} min")
    print("==================================================")
